...
kNN_graph = function(W, K) {
  ...
  if (n-K-1 < 1){
    print("Error: K is too big: n-K-1 < 1. Assuming n > 2, set K = floor(n/2)")
    K = floor(n/2)
  }
...
ANF = function(Wall, K = 20, weight=NULL, type=2, 
               alpha = c(1,1,0,0,0,0,0,0), verbose=F) {
  # sanity check Wall
  if (!is.list(Wall)){
  ...
  if (length(weight) > n) {
    print("WARNING: length(weight) > length(Wall). Discard the last Ws")
    weight = weight[1:n]
  }
  if (length(weight) < n) {
    print("WARNING: length(weight) < length(Wall). Assume the last Ws have 0 weight")
