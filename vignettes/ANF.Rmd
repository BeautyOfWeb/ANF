...
## Load data 
`TCGA_5CancerTypes.RData` (https://github.com/BeautyOfWeb/TCGA_5CancerTypes.RData, to be submitted to bioconductor experiment package) contains three R objects: `Wall`, `project_ids` and `survival.plot`:
...
`survival.plot` is a data.frame containing patient survival data for survival analysis, providing an "indirect" way to evaluate clustering results.

See paper https://arxiv.org/abs/1708.07136 for more explanation.
```{r}
download.file("https://github.com/BeautyOfWeb/TCGA_5CancerTypes.RData/raw/master/TCGA_5CancerTypes.RData", destfile = "TCGA_5CancerTypes.RData")
load("TCGA_5CancerTypes.RData")
